...
def fit_square(left, top, delta, H, W, color=None, allow_shrink=True):
    if allow_shrink:
        left = max(0, left)
        top = max(0, top)
        ...
        if W >= delta:
            ...
            left = left - (left + delta - W) // 2
        if H >= delta:
            top = max(0, top)
            if top + delta >= H:
                top = H - 1 - delta
        else:
            top = top - (top + delta - H) // 2
        return patches.Rectangle((left, top), delta, delta, edgecolor=color, fill=False)
...
def image_selector(image, annotation, species, image_path, annotation_path):
    ...
    rect1 = fit_square(bbox['left'] - (delta - bbox['right'] + bbox['left'])//2,
                       bbox['top'] - (delta - bbox['bottom'] + bbox['top'])//2,
                       delta, H, W, color='b')
    ax.add_patch(rect1)

    # Draw increased bounding box
    rect2 = fit_square(bbox['left'] - delta // 5,
                       bbox['top'] - delta // 5,
...
def dataset_iterator(caltech_path, caltech_new_path, caltech_new_dir):
    global i, i0
    ...
    caltech_new_annotation_path = os.path.join(caltech_new_path, caltech_new_dir)
    ...
    for species in os.listdir(caltech_images_path):
        flag = None
        ...
        if species[:2] == '._':  # Clean images directory:
            ...
            for image_name in os.listdir(species_images_path):
                image_path = os.path.join(species_images_path, image_name)
                if image_name[:2] == '._':  # Clean images directory:
                    os.remove(image_path)
                else:
                    if i < i0:
                        i += 1
                    else:
                        if flag != -1:  # flap == -1 skips species
                            # Load image and annotation
                            image = imread(image_path)
                            annotation_path = os.path.join(species_annotation_path, image_name.split('.')[0] + '.mat')
                            annotation = loadmat(annotation_path, squeeze_me=True)

                            # Save image and annotation
                            #new_image_path = os.path.join(species_new_images_path, image_name)
                            new_annotation_path = os.path.join(species_new_annotation_path, image_name.split('.')[0] + '.pickle')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--caltech_path', required=False, default='C:/Datasets/Caltech-UCSD-Birds-200',
                        help='path to Caltech UCSD dataset')
    parser.add_argument('--data_type', required=False, default='train',
                        help='type of data (train/test)')
    parser.add_argument('--caltech_new_path', required=False, default='C:/Datasets/Caltech-UCSD-Birds-200',
                        help='path to folder where to save dataset')
    parser.add_argument('--caltech_new_dir', required=False, default='annotations-pickle',
                        help='name of folder where to save dataset')
    parser.add_argument('--i0', type=int, dest='i0', required=False, default=5909,
    ...
    dataset_iterator(args.caltech_path, args.caltech_new_path, args.caltech_new_dir)
    print('done')
