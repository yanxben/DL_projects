...
if __name__ == "__main__":
    ...
    def policy(models):
        if len(models) == 1:
            return lambda x: models[0](x)
        else:
            return lambda x: (models[0](x) + models[1](x) + models[2](x) + models[3](x)) / 4

    if ENSEMBLE:
        list_models = [models[7], models[10], models[12], models[13]]
        scores = np.zeros([1, num_models+1])
        for i in range(num_models):
            scores[0, i] = play_game(env, policy(list_models), policy([models[i]])) - \
                                    play_game(env, policy([models[i]]), policy(list_models))
        scores[0,num_models] = np.sum(scores[:,:num_models], axis=1) / (num_models)
    else:
        scores = np.zeros([num_models + 1, num_models + 1])
        for i in range(num_models):
            for j in range(i, num_models):
                scores[i, j] = play_game(env, policy([models[i]]), policy([models[j]])) - \
                               play_game(env, policy([models[j]]), policy([models[i]]))
    ...
    if not ENSEMBLE:
        ...
        plt.yticks(range(1), ['ENSEMBLE'])
        plt.text(num_models - 0.3, 0, '{:.2f}'.format(scores[0, num_models]), fontsize=12, color='w')
