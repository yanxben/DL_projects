...
def save_end_game_data(data, labels, win_labels, lose_labels):
    print('-data-')
    pd.DataFrame(data.reshape([data.shape[0], BOARD_H * BOARD_W * 3])).to_csv('./data/supervised_images/images_df.csv')
    print('-labels-')
    pd.DataFrame(labels).to_csv('./data/supervised_images/labels_df.csv')
    print('-win_labels-')
    pd.DataFrame(win_labels).to_csv('./data/supervised_images/win_labels_df.csv')
    print('-lose_labels-')
    pd.DataFrame(lose_labels).to_csv('./data/supervised_images/lose_labels_df.csv')